...
app.get("/cars/:brand", (req, res) => {
    ...
    res.status(200).send({ "models": cars[brand] });
})

app.get("/api/users/all", (req, res) => { res.status(200).send({ users: data }) });
app.get("/api/users/car/:car", (req, res) => {
    const car = req.params.car;

    res.status(200).send({ users: data.filter(user => user.car.toLowerCase() === car.toLowerCase()) });
});

app.get("/api/users/id/:id", (req, res) => {
    const searchId = parseInt(req.params.id);
    res.status(200).send({ users: data.filter(user => user.id === searchId) });
});

app.get("/api/users-params/fullnames/all", (req, res) => {
    res.status(200).send({ fullNames: data.map((user) => user = user['first_name'] + " " + user['last_name']) });
});
...
// const uri = `mongodb+srv://ErnestasPETK:${process.env.MONGO_DB_PASS}@ubiquitous-engine.3r19y.mongodb.net/?retryWrites=true&w=majority`;
// const client = new MongoClient(uri, { useNewUrlParser: true, useUnifiedTopology: true, serverApi: ServerApiVersion.v1 });
// client.connect(err => {
//     const collection = client.db("nodeJs").collection("cars");
